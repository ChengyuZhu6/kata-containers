...
function create_loop_device(){
	...
	exec_host $node "sudo dd if=/dev/zero of=$loop_file bs=1M count=2500"
	exec_host $node "sudo losetup -fP $loop_file >/dev/null 2>&1"
	local device=$(exec_host $node "sudo losetup -j $loop_file | awk -F'[: ]' '{print \$1}'")
	echo $device
}
...
function cleanup_loop_device(){
	...
	local existed_devices=$(exec_host $node "sudo losetup -j $loop_file | awk -F'[: ]' '{print $1}'")
	
	if [ -n "$existed_devices" ]; then
		# Iterate over each found loop device and detach it
		for d in $existed_devices; do
			exec_host $node "sudo losetup -d "$d" >/dev/null 2>&1"
		done
	fi

	exec_host $node "sudo rm -f "$loop_file""
}