...
Currently, the image pulled in the guest will be downloaded and unpacked in the `/run/kata-containers/image` directory. However, by default, systemd allocates 10% of the available physical RAM to the `/run` directory using a `tmpfs` filesystem. As we all know, memory is valuable, especially for confidential containers. This means that if we run a kata container with the default configuration (where the default memory assigned for a VM is 2048 MiB), `/run` would be allocated around 200 MiB. Consequently, we can only pull images up to 200 MiB in the guest. So we can use a block volume from the host and use dm-crypt and dm-integrity to encrypt the block volume in the guest, providing a secure place to store downloaded container images.
...
```bash
$ loop_file="/tmp/trusted-storage.img"
...
spec:
  ...
  containers:
    - name: app-container
      image: ghcr.io/confidential-containers/test-container:big-size
      command: ["/bin/sh", "-c"]
      args:
        - sleep 6000
      volumeDevices:
...
5. Docker image size
```bash
docker image ls|grep "big-size"
ghcr.io/confidential-containers/test-container               big-size            385d98c8996f   20 months ago   965MB
```
...
vda                                                   
└─ephemeral_image_encrypted_disk
                                        333.5M    70% /run/kata-containers/image

$ cryptsetup status ephemeral_image_encrypted_disk
/dev/mapper/ephemeral_image_encrypted_disk is active and is in use.
  ...
  size:    3039232 sectors
  mode:    read/write

$ mount|grep "ephemeral_image_encrypted_disk"
/dev/mapper/ephemeral_image_encrypted_disk on /run/kata-containers/image type ext4

$ du -h --max-depth=1 /run/kata-containers/image/
60K     /run/kata-containers/image/overlay
16K     /run/kata-containers/image/lost+found
1001M   /run/kata-containers/image/layers
1001M   /run/kata-containers/image/

$ free -m
              total        used        free      shared  buff/cache   available
Mem:           1989          50         798           0        1140        1905
Swap:             0           0           0
```